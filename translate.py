...
for node in data['nodes']:
    ...
    elif type in ['String', 'Float', 'Boolean', 'Assert', 'Not']:
        value = node['fields']['in'][0]['val']
    elif type == "Color":
        value = ','.join([str(value['val']) for value in node['fields']['in']])
    ...
    elif type == "PasswordDialog":
        value = dict([(item['name'], item['val']) for item in node['fields']['in']])
    else:
        value = ''
...
for mod in modules:
    ...
    if mod.type == 'Boolean':
        add_value = True
        add_type = 'org.vistrails.vistrails.basic:Boolean'
        add_name = 'value'
    ...
    if mod.type == 'Color':
        add_value = True
        add_type = 'org.vistrails.vistrails.basic:Color'
        add_name = 'value'
    ...
    if mod.type == 'Assert' and mod.value != '':
        add_value = True
        add_type = 'org.vistrails.vistrails.basic:Boolean'
        add_name = 'condition'
    if isinstance(mod.value, dict):
        add_multi_value = True
        add_value = True

    if add_value:
        if add_multi_value:
            for (name, value) in mod.value.items():
